...
func NewDescribeNamespacesIterator(
    ...
    it := DescribeNamespacesIterator{
        ...
        pageToken: nil,
        last: false,
        result: make([]stamina.Namespace, 0),
...
) (stamina.Namespace, error) {
    if len(p.result) == 0 && !p.last {
        err := p.load()
        if err != nil {
            return stamina.Namespace{}, err
        }
    }
    if len(p.result) == 0 {
        return stamina.Namespace{}, errors.New("out of range")
    }
    ret := p.result[0]
    p.result = p.result[1:]
    if len(p.result) == 0 && !p.last {
