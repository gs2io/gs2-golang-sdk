...
type DescribeRateModelMastersIterator struct {
    ...
    pageToken *string
    last bool
    result []enhance.RateModelMaster
...
func NewDescribeRateModelMastersIterator(
    ...
    it := DescribeRateModelMastersIterator{
        ...
        pageToken: nil,
        last: false,
        result: make([]enhance.RateModelMaster, 0),
...
) (enhance.RateModelMaster, error) {
    if len(p.result) == 0 && !p.last {
        err := p.load()
        if err != nil {
            return enhance.RateModelMaster{}, err
        }
    }
    if len(p.result) == 0 {
        return enhance.RateModelMaster{}, errors.New("out of range")
    }
    ret := p.result[0]
    p.result = p.result[1:]
    if len(p.result) == 0 && !p.last {
