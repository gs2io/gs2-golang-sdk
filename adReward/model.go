...
func NewHistoryFromJson(data string) History {
	dict := map[string]interface{}{}
	_ = json.Unmarshal([]byte(data), &dict)
	return NewHistoryFromDict(dict)
}

func NewHistoryFromDict(data map[string]interface{}) History {
	return History{
		Provider:      core.CastString(data["provider"]),
...
func (p History) ToDict() map[string]interface{} {
...
func (p History) Pointer() *History {
	return &p
}

func CastHistories(data []interface{}) []History {
	v := make([]History, 0)
	for _, d := range data {
		v = append(v, NewHistoryFromDict(d.(map[string]interface{})))
	}
	return v
}

func CastHistoriesFromDict(data []History) []interface{} {
	v := make([]interface{}, 0)
	for _, d := range data {
		v = append(v, d.ToDict())
	}
