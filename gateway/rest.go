...
func getWebSocketSessionAsyncHandler(
	client Gs2GatewayRestClient,
	job *core.NetworkJob,
...
func (p Gs2GatewayRestClient) GetWebSocketSessionAsync(
	...
	go getWebSocketSessionAsyncHandler(
		p,
		&core.NetworkJob{
			Url:          p.Session.EndpointHost("gateway").AppendPath(path, replacer),
			Method:       core.Get,
...
func (p Gs2GatewayRestClient) GetWebSocketSession(
	request *GetWebSocketSessionRequest,
...
func getWebSocketSessionByConnectionIdAsyncHandler(
	client Gs2GatewayRestClient,
	job *core.NetworkJob,
...
func (p Gs2GatewayRestClient) GetWebSocketSessionByConnectionIdAsync(
	request *GetWebSocketSessionByConnectionIdRequest,
	callback chan<- GetWebSocketSessionByConnectionIdAsyncResult,
) {
	path := "/{namespaceName}/session/{connectionId}"
    if request.NamespaceName != nil && *request.NamespaceName != ""  {
        path = strings.ReplaceAll(path, "{namespaceName}", core.ToString(*request.NamespaceName))
    } else {
        path = strings.ReplaceAll(path, "{namespaceName}", "null")
    ...
    if request.ConnectionId != nil && *request.ConnectionId != ""  {
        path = strings.ReplaceAll(path, "{connectionId}", core.ToString(*request.ConnectionId))
    } else {
        path = strings.ReplaceAll(path, "{connectionId}", "null")
    }
	...
	go getWebSocketSessionByConnectionIdAsyncHandler(
		p,
		&core.NetworkJob{
			Url:          p.Session.EndpointHost("gateway").AppendPath(path, replacer),
			Method:       core.Get,
...
func (p Gs2GatewayRestClient) GetWebSocketSessionByConnectionId(
	request *GetWebSocketSessionByConnectionIdRequest,
