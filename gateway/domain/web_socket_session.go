...
type WebSocketSessionDomain struct {
...
func NewWebSocketSessionDomain(
    session *core.Gs2RestSession,
    webSocketSessionCache cache.WebSocketSessionDomainCache,
    namespaceName string,
    userId string,
...
func (p WebSocketSessionDomain) SetUserId(
    request gateway.SetUserIdByUserIdRequest,
) (*gateway.SetUserIdByUserIdResult, error) {
    request.NamespaceName = &p.namespaceName
    request.UserId = &p.userId
    r, err := p.client.SetUserIdByUserId(
        &request,
    )
    if err != nil {
        return nil, err
...
func (p WebSocketSessionDomain) SendNotification(
    request gateway.SendNotificationRequest,
) (*gateway.SendNotificationResult, error) {
    request.NamespaceName = &p.namespaceName
    request.UserId = &p.userId
    r, err := p.client.SendNotification(
        &request,
    )
    if err != nil {
        return nil, err
