...
func NewDescribeNamespacesIterator(
    ...
    it := DescribeNamespacesIterator{
        ...
        pageToken: nil,
        last: false,
        result: make([]job_queue.Namespace, 0),
...
) (job_queue.Namespace, error) {
    if len(p.result) == 0 && !p.last {
        err := p.load()
        if err != nil {
            return job_queue.Namespace{}, err
        }
    }
    if len(p.result) == 0 {
        return job_queue.Namespace{}, errors.New("out of range")
    }
    ret := p.result[0]
    p.result = p.result[1:]
    if len(p.result) == 0 && !p.last {
