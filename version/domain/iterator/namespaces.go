...
func NewDescribeNamespacesIterator(
    ...
    it := DescribeNamespacesIterator{
        ...
        pageToken: nil,
        last: false,
        result: make([]version.Namespace, 0),
...
) (version.Namespace, error) {
    if len(p.result) == 0 && !p.last {
        err := p.load()
        if err != nil {
            return version.Namespace{}, err
        }
    }
    if len(p.result) == 0 {
        return version.Namespace{}, errors.New("out of range")
    }
    ret := p.result[0]
    p.result = p.result[1:]
    if len(p.result) == 0 && !p.last {
