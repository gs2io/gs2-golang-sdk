...
type VersionModelDomain struct {
...
func (p VersionModelDomain) Load(
    request version.GetVersionModelRequest,
) (*version.GetVersionModelResult, error) {
    request.NamespaceName = &p.namespaceName
    request.VersionName = &p.versionName
    r, err := p.client.GetVersionModel(
        &request,
    )
    if err != nil {
        return nil, err
    }
    p.versionModelCache.Update(*r.Item)
    return r, nil
}
