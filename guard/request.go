...
func (p *WaitDeployRequest) UnmarshalJSON(data []byte) error {
	str := string(data)
	if len(str) == 0 {
		*p = WaitDeployRequest{}
		return nil
...
func NewWaitDeployRequestFromJson(data string) (WaitDeployRequest, error) {
	req := WaitDeployRequest{}
	err := json.Unmarshal([]byte(data), &req)
	if err != nil {
		return WaitDeployRequest{}, err
	}
	return req, nil
}

func NewWaitDeployRequestFromDict(data map[string]interface{}) WaitDeployRequest {
	return WaitDeployRequest{
		NamespaceName: func() *string {
			v, ok := data["namespaceName"]
			if !ok || v == nil {
				return nil
...
func (p WaitDeployRequest) ToDict() map[string]interface{} {
	return map[string]interface{}{
		"namespaceName": p.NamespaceName,
	}
}

func (p WaitDeployRequest) Pointer() *WaitDeployRequest {
	return &p
}
