...
func NewDescribeNamespacesIterator(
    ...
    it := DescribeNamespacesIterator{
        ...
        pageToken: nil,
        last: false,
        result: make([]showcase.Namespace, 0),
...
) (showcase.Namespace, error) {
    if len(p.result) == 0 && !p.last {
        err := p.load()
        if err != nil {
            return showcase.Namespace{}, err
        }
    }
    if len(p.result) == 0 {
        return showcase.Namespace{}, errors.New("out of range")
    }
    ret := p.result[0]
    p.result = p.result[1:]
    if len(p.result) == 0 && !p.last {
