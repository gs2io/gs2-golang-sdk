...
type DescribeTriggersIterator struct {
...
func NewDescribeTriggersIterator(
    triggerCache cache.TriggerDomainCache,
    client schedule.Gs2ScheduleRestClient,
    namespaceName string,
    accessToken auth.AccessToken,
) DescribeTriggersIterator {
    it := DescribeTriggersIterator{
...
func (p *DescribeTriggersIterator) load() error {
    r, err := p.client.DescribeTriggers(
...
func (p *DescribeTriggersIterator) HasNext(

) bool {
    return len(p.result) != 0 || !p.last
}

func (p *DescribeTriggersIterator) Next(
