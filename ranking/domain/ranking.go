...
func (p RankingDomain) Load(
    request ranking.GetRankingByUserIdRequest,
) (*ranking.GetRankingByUserIdResult, error) {
    request.CategoryName = p.categoryName
    r, err := p.client.GetRankingByUserId(
        &request,
    )
    if err != nil {
        return nil, err
